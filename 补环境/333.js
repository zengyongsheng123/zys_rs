...
obj_toString(HTMLCollection.prototype, 'HTMLCollection')

function HTMLHeadElement(){
    this.childElementCount = 49;
}
obj_toString(HTMLHeadElement.prototype, 'HTMLHeadElement')
...
obj_toString(CanvasRenderingContext2D.prototype, 'CanvasRenderingContext2D')
...
obj_toString(WebGLRenderingContext.prototype, 'WebGLRenderingContext')
...
obj_toString(HTMLCanvasElement.prototype, 'HTMLCanvasElement')
...
obj_toString(Element.prototype, 'Element')
Element = new Element();
...
obj_toString(localStorage, 'localStorage')
...
obj_toString(Location.prototype, 'Location');
window.location = new Location();
location = watch(window.location, 'location');

function MimeTypeArray() {
    this.length = 2;
}
obj_toString(MimeTypeArray.prototype, 'MimeTypeArray');
...
navigator = watch(window.navigator, 'navigator');
...
obj_toString(Screen.prototype, 'Screen');
window.screen = new Screen();
screen = watch(window.screen, 'screen');
...
function generateH5st(paramsH5sign) {
    try {
        const params = paramsH5sign.body;
        paramsH5sign.body = CryptoJS.SHA256(JSON.stringify(params)).toString();
        console.log("paramsH5sign:", paramsH5sign);
        console.log("window.ParamsSign exists:", !!window.ParamsSign);
        window.PSign = new window.ParamsSign({ appId: 'f06cc' });
        const _$Ga = window.PSign._$sdnmd(paramsH5sign);
        return {
            h5st: _$Ga.h5st,
            length: _$Ga.h5st.length,
            timestamp: paramsH5sign.t
        };
    } catch (e) {
        console.error("Error in generateH5st:", e.message);
        return { error: e.message };
    }
}

module.exports = {
    generateH5st: generateH5st
};
console.log(generateH5st());
process.exit()