...
class bezierTrajectory:
        ...
        if cbb != 0:
            ...
            for i in range(cbb):
                if kg == 0:
                    ...
                    kg = 0
                # print(d)
                y = self.trackArray(ends, d, numberListOfcbb, le=2, deviation=0, bias=0, type=0, cbb=0, yhh=0)
                s += list(y['trackArray'])
                ends = d
    ...
    def generate_gj(self, slider_x, slider_y):
        guiji = self.trackArray([1, slider_y], [slider_x, slider_y], slider_x-15, type=0, cbb=1, yhh=2, deviation=0, le=3)['trackArray']
        start_time = int(time.time() * 1000)
        guiji_list = []
        for i in guiji:
            start_time += random.randint(7, 9)
            aa = {'x':int(i[0]), 'y':int(slider_y), 'time': start_time}
            # print(aa)
...
if __name__ == '__main__':
    ...
    print(track_.generate_gj(80,99))
